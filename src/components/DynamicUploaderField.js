    ...
    if (f_proofs.filter((proof) => proof === id).length !== 0) {
      axios
        ...
        .then((res) => {
          setFileState((prev) => ({
            ...prev,
            f_proofs: prev.f_proofs.filter((proof) => proof !== id),
          }));
        })
        .catch((err) => {
          console.log(err);
      ...
      if (extraProofs.length > 1) {
        setFileState((prev) => ({
          ...prev,
          extraProofs: prev.extraProofs.filter((proof) => proof.id !== id),
        }));
      }
    }
    return;
        ...
        <FormControlLabel
          ...
          label={
            ...
            >
              ...
              <Typography className={classes.attachFileLabel}></Typography>
            </Box>
          }
          control={
            <StyledInput
